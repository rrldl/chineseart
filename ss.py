...
class ImageSearchService:
    ...
    def calculate_weighted_similarity(self, base_similarity, node_properties, query_text=None):
        """
        计算加权相似度 (重点修改：直接接收 FAISS 传来的基础相似度，不再去算点积了！)
        保留了所有 Neo4j 朝代、作者、描述打分的精华逻辑！
        """
        similarity = base_similarity # 直接拿 FAISS 算好的余弦距离
    ...
    def _hybrid_search(self, query_emb, top_k=10, query_text=None):
        """
        革命性更新：图谱融合检索 (FAISS 海选 + Neo4j 精确制导)
        """
        self._ensure_database_connected()
        self._ensure_model_loaded()
        
        # 1. 第一阶段：FAISS 毫秒级海选前 50 名
        # 🌟 必须加上 L2 归一化，否则 FAISS 的 IP 索引算出来的不是正确的余弦相似度！
        ...
        # 2. 第二阶段：精准查询 Neo4j，并进行 KG 图谱加权
        for i in range(len(indices[0])):
            idx = indices[0][i]
            if idx == -1: continue
            
            # [桥梁] 获取真实的图片路径和对应的图谱 ID
            img_path = self.paths_array[idx]  # 例如 artworks/legacy0001_0.jpg
            base_id = self.base_ids_list[idx] # 例如 legacy0001
            
            # 基础的视觉/语义余弦相似度
            base_sim = float(distances[0][i])
            
            query = """
            MATCH (art:Artwork {id: $base_id})
            OPTIONAL MATCH (artist:Artist)-[:CREATED]->(art)
            OPTIONAL MATCH (art)-[:BELONGS_TO_DYNASTY]->(dynasty:Dynasty)
            OPTIONAL MATCH (art)-[:HAS_CATEGORY]->(cat:Category)
            RETURN art.name AS title, 
                   art.desc AS description, 
                   art.mediaType AS medium,
                   artist.name AS author, 
                   dynasty.name AS dynasty, 
                   cat.name AS style
            """
            try:
            ...
            # 构造节点属性（必须完全契合你的 calculate_weighted_similarity 期望的格式！）
            if record:
                meta = record[0]
                node_properties = {
                    'title': meta.get('title') or '未知作品',
                    'description': meta.get('description') or '',
                    'dynasty': meta.get('dynasty') or '',
                    'style': meta.get('style') or '', # 把分类(category)映射为你的 style
                ...
                # 即使图谱里没录入，也不能让程序崩溃
                node_properties = {'title': '未录入档案画作', 'description': ''}
                node_id = -1
                
            # 3. 把 FAISS 的纯视觉得分 和 Neo4j 的图谱知识得分 融合
            ...
            if final_similarity > 0.1:
                result_item = {
                    ...
                    'description': node_properties.get('description', ''),
                    # 🌟 直接给前端可用的图片路径
                    'image_url': f"/artwork_image/{img_path.replace(chr(92), '/')}"
                }
                all_results.append(result_item)

        # 4. 第三阶段：综合大排名 (原封不动调用你的排序算法)
        filtered_results = self.filter_and_rank_results(all_results, query_text)
    ...
    def _get_artwork_image_url(self, img_path):
        ...
        try:
            ...
            # 1. 这里的 IMAGE_ROOT_DIR 是你的图片根目录 D:\shuhua_picture\work
            # 我们拼出绝对路径来检查文件是否真的在硬盘上
            # 注意：在 __init__ 里应该定义好 self.image_root_dir = r"D:\shuhua_picture\work"
            full_path = os.path.join(self.image_root_dir, img_path)

            if os.path.exists(full_path):
                # 2. 返回给前端的 URL 路径
                # 将 Windows 的反斜杠 \ 统一替换为网页通用的斜杠 /
...
def _find_image_url_by_id(self, base_id):
    """通过 base_id 在 paths_array 里找图片 URL，支持 artworks/inscriptions 两个子目录"""
    if not self.base_ids_list or not self.paths_array:
        return None
    for i, bid in enumerate(self.base_ids_list):
        if bid == base_id:
            img_path = self.paths_array[i].replace('\\', '/')
            return f"/artwork_image/{img_path}"
    return None
